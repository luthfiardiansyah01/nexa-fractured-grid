export class PuzzleGenerator {
  static generatePuzzle(size = 5) {
    ...
    while (current.x !== end.x || current.y !== end.y) {
      const moves = [];
      if (current.x < end.x) moves.push({ x: current.x + 1, y: current.y });
      if (current.y < end.y) moves.push({ x: current.x, y: current.y + 1 });
      if (current.y > end.y) moves.push({ x: current.x, y: current.y - 1 });
      
      // Add randomness but bias towards goal
      if (Math.random() > 0.7 && current.x > 0) moves.push({ x: current.x - 1, y: current.y });
    ...
    for (let i = 0; i < path.length; i++) {
      ...
      if (!prev) { // Start
        if (next.x > p.x) rotation = 90; // Right
        else if (next.y > p.y) rotation = 180; // Down
        else if (next.y < p.y) rotation = 0; // Up
      } else if (!next) { // End
        if (prev.x < p.x) rotation = 270; // Left input
        else if (prev.y < p.y) rotation = 0; // Top input
        else if (prev.y > p.y) rotation = 180; // Bottom input
      } else {
    ...
    const puzzleGrid = grid.map(row => row.map(cell => ({
      ...cell,
